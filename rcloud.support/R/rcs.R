...
usr.key <- function(..., user=.session$username, notebook=.session$notebook) paste( user, notebook, ..., sep='/', collapse=':')

rcs.get <- function(key, engine=.session$rcs.engine) UseMethod("rcs.get", engine)
