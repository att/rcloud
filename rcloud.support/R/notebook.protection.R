...
rcloud.set.notebook.cryptgroup <- function(notebookid, groupid, modify=TRUE) { # or NULL to decrypt/make public
    ...
    if(!is.notebook.encrypted(notebookid)) resp <- update.solr(rcloud.get.notebook(notebookid),rcloud.notebook.star.count(notebookid))
    invisible(TRUE)
}

is.notebook.encrypted <- function(id)
  sapply(property.multiple.notebooks(id, "cryptgroup"), function(x) {!is.null(x) && (x==TRUE)})
