rcloud.config.get.recent.notebooks <- function() {
  ...
  keys <- rcs.list(usr.key(user=users, notebook="system", "config", "recent", "*"))
...
rcloud.config.set.recent.notebook <- function(id, date) {
  rcs.set(usr.key(user=.users, notebook="system", "config", "recent", id), date)
}

rcloud.config.clear.recent.notebook <- function(id) {
  users <- rcloud.get.users()
  rcs.rm(usr.key(user=users, notebook="system", "config", "recent", id))
}
