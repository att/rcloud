...
rcs.get.RCSredis <- function(key, engine=.session$rcs.engine) .rdo(redisGet(key), engine)

rcs.set.RCSredis <- function(key, value, engine=.session$rcs.engine) .rdo(redisSet(key, value), engine)
