...
RCloud.create = function (rcloud_ocaps) {
    ...
    function rcloud_github_handler(command, promise) {
        ...
        function failure(err) {
            var message = _.isObject(err) && 'ok' in err ? err.content.message : err.toString();
   ...
   ["query_solr"],
        ...
        rcloud.upload_to_notebook = function (force, k) {
            k = k || _.identity;
            var on_success = function (v) {
                k(null, v);
            };
            var on_failure = function (v) {
                k(v, null);
            };
...
ui_utils.fa_button = function (which, title, classname, style) {
    ...
    var span = $.el.span({
            'class': 'fontawesome-button ' + (classname || '')
        },
        icon);
    if(style) {
        for(var k in style)
...
(function () {
    ...
    function create_markdown_cell_html_view(language) {
        return function (cell_model) {
            ...
            ace_div.css({
                'background-color': languages[language]["background-color"]
            });
            ...
            session.on('change', function () {
                notebook_cell_div.css({
                    'height': (ui_utils.ace_editor_height(widget) + 24) + "px"
                });
                widget.resize();
            });
            ...
            var result = {
                ...
                show_source: function () {
                    ...
                    var obj = {
                        'height': (ui_utils.ace_editor_height(widget) + 25) + "px"
                    };
...
Notebook.create_model = function () {
    ...
    return {
        ...
        remove_asset: function (asset_model, n, skip_event) {
            ...
            while(x < this.assets.length && n) {
                if(this.assets[x].filename() == filename) {
                    ...
                    changes.push(that.assets[x].change_object({
                        erase: 1
                    }));
        ...
        remove_cell: function (cell_model, n, skip_event) {
            ...
            while(x < this.notebook.length && n) {
                if(this.notebook[x].id() == id) {
                    ...
                    changes.push(that.notebook[x].change_object({
                        erase: 1
                    }));
        ...
        move_cell: function (cell_model, before) {
            var pre_index = this.notebook.indexOf(cell_model),
                changes = this.remove_cell(cell_model, 1, true)
                    .concat(before >= 0 ? this.insert_cell(cell_model, before, true) : this.append_cell(cell_model, null, true)),
        ...
        change_cell_language: function (cell_model, language) {
            ...
            cell_model.language(language);
            var c = cell_model.change_object({
                language: language
            });
...
Notebook.create_controller = function (model) {
    ...
    function append_cell_helper(content, type, id) {
        ...
        return {
            ...
            changes: model.append_cell(cell_model, id)
        };
    }
    ...
    function insert_cell_helper(content, type, id) {
        ...
        return {
            controller: cell_controller,
            changes: model.insert_cell(cell_model, id)
        };
    }
        ...
        for(var f in nf) {
            ...
            if(f in cf) {
                if(cf[f].language != nf[f].language || cf[f].content != nf[f].content) {
                    changes.push(nf.change_object({
                        id: f
                    }));
                }
                delete cf[f];
            } else changes.push(nf.change_object({
                id: f,
                erase: true
            }));
        }
    ...
    function update_notebook(changes, gistname) {
        ...
        changes = changes.filter(function (change) {
            return !!change.content || change.erase;
        });
        if(!changes.length)
    ...
    function setup_show_source() {
        show_source_checkbox_ = ui_utils.checkbox_menu_item($("#show-source"),
            function () {
                result.show_r_source();
            },
            function () {
                result.hide_r_source();
            });
    ...
    var result = {
        ...
        fork_or_revert_notebook: function (is_mine, gistname, version) {
            ...
            return promiseChanges.spread(function (changes, gistname) {
                return changes.length ? update_notebook(changes, gistname) : that.load_notebook(gistname, null); // do a load - we need to refresh
            });
        },
    ...
    $("#upload-submit").click(function () {
        ...
        function failure() {
            ...
            var overwrite = bootstrap_utils
                ...
                .text("Overwrite");
            p.append(overwrite);
            $("#file-upload-div").append(bootstrap_utils.alert({
                'class': 'alert-danger',
                html: alert_element
            }));
        }

        var upload_function = to_notebook ? rcloud.upload_to_notebook : rcloud.upload_file;
...
RCloud.UI.scratchpad = {
    ...
    init: function () {
        ...
        function setup_scratchpad(div) {
            ...
            session.on('change', function () {
                div.css({
                    'height': ui_utils.ace_editor_height(widget, 30) + "px"
                });
                widget.resize();
            });
...
RCloud.UI.command_prompt = {
    ...
    setup_command_prompt: function () {
        ...
        function set_ace_height() {
            prompt_div.css({
                'height': ui_utils.ace_editor_height(widget) + "px"
            });
            widget.resize();
        }
        prompt_div.css({
...
RCloud.UI.notebook_title = {
    ...
    set: function (text) {
        ...
        while(window.innerWidth - $("#notebook-title").width() < 505) {
            ...
            $("#notebook-title").text((ellipt_start ? '.../' : '') + text +
                (ellipt_end ? '...' : ''));
        }
