var editor = function () {
    ...
    function add_notebook_info(user, gistname, entry) {
        ...
        if (user === username_)
            p = p.then(function () {
                rcloud.config.add_notebook(gistname);
            });
        return p;
    }
    ...
    function compare_nodes(a, b) {
        ...
        if (so) return so;
            ...
            var amatch = trnexp.exec(alab),
                bmatch = trnexp.exec(blab);
...
*/
        ...
        function create_user_book_entry_map(books) {
            ...
            _.each(books,
                function (book) {
                    ...
                    if (!entry.username || entry.username === "undefined" || !entry.description || !entry.last_commit) {
    ...
    function load_tree(root_data) {
        ...
        var alls = _.find(root_data, function (root) {
            return root.id === "/alls";
        }).children;
            ...
            .spread(function (user_notebook_set, my_stars_array) {
                ...
                return Promise.all([rcloud.config.get_current_notebook()
                    ...
                    .then(function (interests) {
                        root_data.push(interests);
                    })
                    .then(populate_all_notebooks.bind(null, user_notebook_set))
                    .then(populate_friends)
                    .spread(function (friends, alls) {
                        root_data.push(friends, alls);
                    })
                    .return(root_data);
            })
            .then(load_tree)
        ...
        function process_history(nshow) {
            ...
            if (debug_colors)
                for (var ii = 0, ee = curr_count(); ii < ee; ++ii)
                    $tree_.tree('updateNode', node.children[ii], {
                        color: ''
                    });
            ...
            if (node.children.length) {
                ...
                nins = find_index(history, function (h) {
                    return h.version == first.version;
                });
                insf = function (dat) {
                    return $tree_.tree('addNodeBefore', dat, first);
                };
            } else {
                nins = nshow;
                insf = function (dat) {
                    return $tree_.tree('appendNode', dat, node);
                };
            }
            ...
            if (count < nshow) { // top up
                if (ellipsis)
                    insf = function (dat) {
                        return $tree_.tree('addNodeBefore', dat, ellipsis);
                    };
                else
                    insf = function (dat) {
                        return $tree_.tree('appendNode', dat, node);
    ...
    function scroll_into_view(node) {
        ...
        if ($(node.element).position().top > height)
            $tree_.parent().scrollTo(null, $tree_.parent().scrollTop() + $(node.element).position().top - height + 50);
        else if ($(node.element).position().top < 0)
            $tree_.parent().scrollTo(null, $tree_.parent().scrollTop() + $(node.element).position().top - 100);
    }
...
//<<<<<<<field
    ...
    function on_create_tree_li(node, $li) {
        ...
        if (node.gistname && !node.version) {
            ...
            var always = $($.el.span({
                'class': 'notebook-commands-right'
            }));
            var add_buttons = adder(always);
            var star_style = _.extend({
                'font-size': '80%'
            }, icon_style);
            var states = {
                true: {
            ...
            star_unstar.click(function (e) {
                ...
                var new_state = !state;
                result.star_notebook(new_state, {
                    gistname: node.gistname,
                    user: node.user
                });
            });
            ...
            var appear = $($.el.span({
                'class': 'notebook-commands appear'
            }));
    ...
    function tree_click(event) {
        if (event.node.id === 'showmore')
            ...
            if (event.click_event.metaKey || event.click_event.ctrlKey)
                ...
                if (event.node.gistname === current_.notebook && event.node.version == current_.version) // nulliness ok here
    ...
    var result = {
        ...
        star_notebook: function (star, opts) {
            ...
            opts = opts || {};
            var gistname = opts.gistname || opts.notebook && opts.notebook.id || current_.notebook;
            var user = opts.user || opts.notebook && opts.notebook.user && opts.notebook.user.login || notebook_info_[gistname].username;
        ...
        fork_notebook: function (is_mine, gistname, version) {
            return shell.fork_notebook(is_mine, gistname, version)
                .bind(this)
                .then(function (notebook) {
                    return this.star_notebook(true, {
                            notebook: notebook,
                            make_current: true,
                            is_change: !!version,
                            version: null
        ...
        load_callback: function (opts) {
            ...
            return function (result) {
                ...
                var stars_promise = (_.has(num_stars_, result.id) ? Promise.resolve(undefined) : rcloud.stars.get_notebook_star_count(result.id).then(function (count) {
