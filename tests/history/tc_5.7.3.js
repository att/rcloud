 ...
 * On selecting the option for 'Revert' present on the top-left corner of the page, the respective version of the 
 * notebook gets saved as the current version of that particular notebook
 */

//Begin Tests
casper.test.begin(" Revert a particular History Version", 13, function suite(test) {
	...
	//Clicking on Revert back icon
	casper.then(function(){
		this.click('#revert-notebook');
		console.log("Clicking on Revert back icon");
		this.wait(5000);
		this.test.assertExists({type:'css', path:'.icon-edit'},"Toggle edit icon is present");//Verifying whether Toggle edit button exists after clicking on revert back
	});
